					       ...
					       KeyModifiers unmaskedMod)
{
    ...
    KSym kSym =
	ibus_chewing_pre_edit_key_code_to_key_sym(self->icPreEdit, keySym, keycode,
						  unmaskedMod);
					   ...
					   guint prop_state)
{
	...
	if (ibus_chewing_pre_edit_has_flag(self->icPreEdit, FLAG_SYNC_FROM_IM)
		|| ibus_chewing_pre_edit_has_flag(self->icPreEdit, FLAG_SYNC_FROM_KEYBOARD)){
	...
	chewing_set_ShapeMode(self->icPreEdit->context,
			      !chewing_get_ShapeMode(self->icPreEdit->
						     context));
	...
	if (ibus_chewing_property_get_state(self->settings_prop) ==
		...
		MakerDialog *mDialog = maker_dialog_new_full
		    (self->icPreEdit->iProperties->properties, _("Setting"),
