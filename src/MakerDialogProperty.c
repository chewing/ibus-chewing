...
static gboolean property_context_from_string(PropertyContext *ctx,
                                             const gchar *str) {
    if (ctx == NULL) {
        return FALSE;
    }
...
void property_context_default(PropertyContext *ctx) {
    ...
    if (!ret) {
        mkdg_log(WARN,
                 "property_context_default(%s): failed to convert string %s, "
                 "return NULL",
...
GValue *property_context_read(PropertyContext *ctx, gpointer userData) {
    ...
    if (mkdg_has_flag(ctx->spec->propertyFlags,
                      MKDG_PROPERTY_FLAG_NO_BACKEND)) {
        return NULL;
    }
    if (ctx->backend == NULL) {
        return NULL;
    ...
    GValue *result =
        mkdg_backend_read(ctx->backend, &(ctx->value), ctx->spec->subSection,
...
GValue *property_context_get(PropertyContext *ctx) {
    ...
    mkdg_log(DEBUG, "property_context_get(%s): value=%s", ctx->spec->key,
             mkdg_g_value_to_string(&(ctx->value)));
    return &(ctx->value);
}
...
gboolean property_context_apply(PropertyContext *ctx, gpointer userData) {
    ...
    mkdg_log(DEBUG, "property_context_apply(%s,-): value %s", ctx->spec->key,
             mkdg_g_value_to_string(&(ctx->value)));
...
MkdgProperties *mkdg_properties_from_spec_array(MkdgPropertySpec specs[],
    ...
    for (i = 0; i < arraySize; i++) {
        PropertyContext *ctx =
            property_context_new(&specs[i], backend, parent, auxData);
...
PropertyContext *mkdg_properties_index(MkdgProperties *properties,
                                       guint index) {
    return (PropertyContext *)g_ptr_array_index(properties->contexts, index);
}
